...
func %s() *%sTag {
	return &%sTag{Component: &Component{WebComponent: &lx.WebComponent{Name: "%s", Attributes: &lx.Attributes{}, Children: nil}}}
}

%s`, name, comment, name, name, name, strings.ToLower(name), TagStruct(name))
	}
	return fmt.Sprintf(`//%s - %s
func %s(tags ...lx.Content) *%sTag {
	return &%sTag{Component: &Component{WebComponent: &lx.WebComponent{Name: "%s", Attributes: &lx.Attributes{}, Children: &tags}}}
}
